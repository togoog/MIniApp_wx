export default class weCropper {
    ...
    init() {
        ...
        wx.getImageInfo({
            ...
            success(res) {
                ...
                self.innerAspectRadio = res.width / res.height;
                let px2r = windowWidth / 750;
                self.croperTarget  = src
                if (self.innerAspectRadio < 1) {
                    self.baseWidth = width * px2r
                    self.baseHeight = width / self.innerAspectRadio * px2r
                } else {
                    self.baseHeight = height * px2r
                    self.baseWidth = height * self.innerAspectRadio * px2r
                }
                self.rectX = (width * windowWidth / 750 - self.baseWidth) / 2
    ...
    touchMove(e) {
        let self = this
        let { minScale, maxScale,width,height } = self
        let [touch0, touch1] = e.touches
        ...
        if (e.touches.length === 1) {
            ...
            if(self.innerAspectRadio <=1){
                self.imgLeft =(self.scaleWidth * self.oldScale == self.baseWidth )? 0: self.rectX + xMove
                self.imgTop = self.rectY + yMove
            }else{
                self.imgLeft = self.rectX + xMove
                self.imgTop = (self.scaleHeight * self.oldScale == self.baseHeight )? 0:self.rectY + yMove
            }
